...
def key_calc_score(input_file_path,filename):
    ...
    scores = []
    scores.append(vggnet_score_cosine)
    ...
    shape_rank_inv = 0
    if vggnet_score_cosine !=0 :
        nn_rank_inv = (1/vggnet_score_cosine)
    ...
    return {'filename':filename,'VGGNet' : vggnet_score_cosine,'Color':colorhist_score_chisquared,'LBP':lbp_cosine,'Shape':shape_score,'IRP':irp_score,'Prod':prod}
...
my_df.to_csv('vgg_culm_1-1.csv', float_format='%g',index=False, header=False)
